    ...
    if (($_FILES["file"]["type"] == "application/vnd.ms-excel") || ($_FILES["file"]["type"] == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")) {
        if ($_FILES["file"]["error"] > 0) {
            ...
            if ($_FILES["file"]["type"] == "application/vnd.ms-excel") {
                move_uploaded_file($_FILES["file"]["tmp_name"], $upload_dir . "/" . '2003_per_pack.xls');
                $fn = $upload_dir . '/2003_per_pack.xls';
            } else {
                move_uploaded_file($_FILES["file"]["tmp_name"], $upload_dir . "/" . '2007_per_pack.xlsx');
                $fn = $upload_dir . '/2007_per_pack.xlsx';
            }
...
elseif (isset($_POST['import_file'])) {
    ...
    $total = input_excel($_SESSION['per_pack']['file'], $acc_prefix);
    ...
    $row_no = 0;
        ...
        foreach ($sheetData as $value) {
            if ($value['A'] == '品名' && $value['B'] == '件双') {
                ...
                $flag = FALSE;
                $flag = $wpdb->update( "{$acc_prefix}goods_name", array( 'gn_per_pack' => $value['B'] ), array( 'gn_name' => $value['A'] ), array( '%d' ), array( '%s' ) );
                if($flag) {
                    echo ++$row_no . ', ' . $value['A'] . ' : ' . $value['B'] . '<br />';
                }
            }
        }
...
function input_form() {
    ...
    <div class="manage-menus">
        <div class="alignleft actions">
            ...
            <input type="button" name="per_pack_return" id="per_pack_return" class="button button-primary" value="取消并返回上级" 
